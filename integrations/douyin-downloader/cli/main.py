...
def _cookie_manager_for_config(config: ConfigLoader) -> CookieManager:
    if config.config_path:
        cookie_file = Path(config.config_path).resolve().parent / ".cookies.json"
...
async def main_async(args):
    ...
    if args.hot_board is not None or args.search:
        discovery_cm = _cookie_manager_for_config(config)
    ...
    cookie_manager = _cookie_manager_for_config(config)
